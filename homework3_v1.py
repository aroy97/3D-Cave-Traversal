import filecmp, math, heapq, time, os
...
file_to_read_as_input = os.getcwd() + '/test_cases/input4.txt'
information_in_file = list()
file_to_check_with_output = os.getcwd() + '/test_results/output4.txt'
...
def organize_output_pattern(visited_locations, destination_location, method_of_traversal):
    ...
    if method_of_traversal == 'BFS':
        ...
        output_file_to_create_ucs_or_astar = open('output.txt', 'w')
...
print(f'time to run algorithm: {code_execution_end_time - code_execution_start_time}')
if locations_covered == 'FAIL':
    output_file_to_create = open('output.txt', 'w')
...
print(filecmp.cmp('output.txt', file_to_check_with_output))
